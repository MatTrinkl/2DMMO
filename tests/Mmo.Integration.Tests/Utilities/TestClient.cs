...
public class TestClient : IDisposable
{
    ...
    public async Task<LoginResponse?> LoginAsync(string username, string password, TimeSpan timeout)
    {
        ...
        LoginResponse? response = await WaitForMessageAsync<LoginResponse>(timeout);
    ...
    private async Task ReceiveLoop(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested && IsConnected)
            {
                ...
                {
                    int read = await _stream.ReadAsync(headerBuffer.AsMemory(headerBytesRead, 4 - headerBytesRead),
                        cancellationToken);
                    if (read == 0)
                ...
                try
                {
                    INetworkMessage message = MessageSerializer.Deserialize(payloadBuffer);
