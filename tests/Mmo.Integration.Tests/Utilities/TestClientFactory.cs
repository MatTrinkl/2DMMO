...
public class TestClientFactory : IDisposable
{
    ...
    public void Dispose()
    {
        foreach (TestClient client in _clients) client.Dispose();
    ...
    public async Task<TestClient> CreateAndConnectAsync(TimeSpan? timeout = null)
    {
        ...
        bool connected = await client.ConnectAsync(timeout ?? TimeSpan.FromSeconds(5));
    ...
    {
        TestClient client = await CreateAndConnectAsync(timeout);

        username ??= $"test_user_{Interlocked.Increment(ref _clientCounter)}";
        password ??= "test_password";

        LoginResponse? loginResponse = await client.LoginAsync(username, password, timeout ?? TimeSpan.FromSeconds(5));
    ...
    public async Task<List<TestClient>> CreateMultipleClientsAsync(int count, TimeSpan? timeout = null)
    {
        ...
        for (int i = 0; i < count; i++) tasks.Add(CreateAuthenticatedClientAsync(timeout: timeout));
