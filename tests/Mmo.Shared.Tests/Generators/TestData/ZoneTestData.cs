...
{
    [BaseData] public ushort Id { get; init; }

    [BaseData] public string Name { get; init; } = "";

    [OptionalData("ZoneListEntry", "ZoneDungeonEntry")]
    public byte RecommendedLevel { get; init; }

    [OptionalData("ZoneMapEntry")] public bool IsDiscovered { get; init; }

    [OptionalData("ZoneDungeonEntry")] public ushort MaxPlayers { get; init; }

    [OptionalData("ZoneListEntry")] public int? Cost { get; init; }

    // No attribute = ignored
    public ZoneBoundsTestData Bounds { get; init; }

    [IgnoreData] public HashSet<Guid> EntityIds { get; init; } = new();
}
