...
public class NpcEntity : CombatEntity, INpcEntity
{
    ...
    [IgnoreMember]
    public override bool IsTrulyPersistent => false; // NPCs werden nicht in DB gespeichert
    ...
    [Key(20)]
    public CombatRank CombatRank { get; set; } = CombatRank.Normal;

    [Key(21)]
    public NpcFunction Function { get; set; } = NpcFunction. Hostile;
    ...
    [Key(24)]
    public float LeashRadius { get; set; } = 40f;

    [Key(25)]
    public bool CanFlee { get; set; } = false;

    [Key(26)]
    public float FleeHealthPercent { get; set; } = 0.15f;

    [Key(27)]
    public bool CanCallForHelp { get; set; } = true;

    [Key(28)]
    public float CallForHelpRadius { get; set; } = 15f;
    ...
    [Key(32)]
    public bool HasPatrolPath { get; set; } = false;
    ...
    [Key(35)]
    public bool IsWaitingAtWaypoint { get; set; } = false;
    ...
    [Key(43)]
    public bool IsQuestGiver { get; set; } = false;
    ...
    public void RemoveThreat(Guid entityId)
    {
        _threatTable.Remove(entityId);
    }
    ...
    public int GetThreat(Guid entityId)
    {
        return _threatTable. GetValueOrDefault(entityId, 0);
    }
