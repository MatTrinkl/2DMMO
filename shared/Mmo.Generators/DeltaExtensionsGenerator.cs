...
public class DeltaExtensionsGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateUnifiedExtensionMethod(INamedTypeSymbol typeSymbol, GeneratorHelpers.DirtyTrackingConfig config,
        List<GeneratorHelpers.TrackedPropertyInfo> properties)
    {
        ...
        sb.AppendLine($"    /// </summary>");
        sb.AppendLine($"    public static {namespaceName}.Dtos.{deltaName} ToDelta(this {typeSymbol.ToDisplayString()} entity)");
        sb.AppendLine("    {");
            ...
            if (unionMemberAttr != null && unionMemberAttr.ConstructorArguments.Length >= 2)
            {
                ...
                if (unionTypeArg.Value is INamedTypeSymbol unionRootType)
                {
                    ...
                    if (unionAttr != null)
                    {
                        ...
                        sb.AppendLine($"    /// This allows different entity types to be stored in the same collection.");
                        sb.AppendLine($"    /// </summary>");
                        sb.AppendLine($"    public static {unionNs}.{unionName} ToDeltaUnion(this {typeSymbol.ToDisplayString()} entity)");
                        sb.AppendLine("    {");
