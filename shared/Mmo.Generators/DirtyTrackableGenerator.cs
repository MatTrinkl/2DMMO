...
public class DirtyTrackableGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateIDirtyTrackableImplementation(INamedTypeSymbol typeSymbol)
    {
        ...
        // Get the GenerateDirtyTracking attribute to find the FlagsEnumType
        var attribute = typeSymbol.GetAttributes()
            .FirstOrDefault(a => a.AttributeClass?.Name == "GenerateDirtyTrackingAttribute");
        
        // Default to EntityDirtyFlags if not specified
        string flagsEnumType = "Mmo.Shared.Entities.Enums.EntityDirtyFlags";
        
        if (attribute != null)
        {
            // Try to get FlagsEnumType from named arguments
        ...
        sb.AppendLine();
        sb.AppendLine($"    public {flagsEnumType} DirtyFlags => _dirtyFlags;");
        sb.AppendLine($"    public bool IsDirty => _dirtyFlags != {flagsEnumType}.None;");
        sb.AppendLine();
        sb.AppendLine($"    public void ClearDirtyFlags() => _dirtyFlags = {flagsEnumType}.None;");
        sb.AppendLine($"    public void MarkDirty({flagsEnumType} flags) => _dirtyFlags |= flags;");
        sb.AppendLine("}");
